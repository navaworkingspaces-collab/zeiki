...
// biometría + unlock, HDU-006 splash real, HDU-007 reset password
// flow con deep links).
import 'dart:async';
//
// Cambios de HDU-005:
...
//   - **1 ruta nueva para deep link de Supabase:**
//     `/auth/reset-password` (AC8). La pantalla de verify-email
//     (AC3) se removió después — ver `chore/cleanup-verify-email-screen`
//     y `docs/current-state.md`.
//   - **Es terminal** (como `/unlock`): el redirect NO la redirige,
...
/// `resetPassword` para el deep link de Supabase. (La ruta de
/// `verifyEmail` se removió después — Supabase no emite el evento de
/// "verify email exitoso", así que la pantalla nunca se muestra.
///
/// Ver `chore/cleanup-verify-email-screen` para el detalle.)
enum AppRoute {
  splash('/splash'),
...
/// `/unlock` y la ruta de HDU-007 siempre son terminales.
