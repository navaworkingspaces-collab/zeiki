...
void main() {
  ...
  group('render (AC18)', () {
    testWidgets('muestra el email del usuario actual', (tester) async {
      ...
      expect(find.byKey(const Key('home_signout')), findsOneWidget);
    });
  ...
  group('sign out (AC19)', () {
    testWidgets('tap en "Salir" → llama signOut y navega a /login',
        (tester) async {
      fakeAuth.session = _makeSession(email: 'hugo@zeiki.app');
      router.go(AppRoute.home.path);
      await pumpHome(tester);
      ...
      await tester.tap(find.byKey(const Key('home_signout')));
      await tester.pumpAndSettle();
