...
**Última actualización:** 2026-07-29 (post-HDU-001).
...
- Crea los agentes `seiki-implementer` y `seiki-auditor` en `C:\Users\Pc\.minimax\agents\`.
...
- **HDU-002:** `seiki-reviewer` — agente de code review formal (3 gates: clean code + security + architecture). Workflow §7 lo referencia. Útil para HDUs de criticidad media-alta.
...
- **HDU-004 (Fase 1):** Splash con feature flags. Depende de HDU-003 (necesita Auth para saber a dónde navegar).

## 🐛 Follow-ups activos (de HDUs cerradas)

| # | Origen | Descripción | Prioridad | Estado |
|---|--------|-------------|-----------|--------|
| 1 | HDU-001 | Crear `seiki-reviewer` (code review 3 gates). | media | pendiente |
